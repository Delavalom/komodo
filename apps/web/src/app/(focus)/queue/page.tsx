...
import { CHIP_CLASS, KIND_TINT, REPLY_TINT } from "@/components/ui";
import { estimateTime, loadQueue } from "@/lib/queue";
import { syncOpenThreads } from "@/lib/sync";
import { JudgeHeader } from "../judge-header";
import { QueueShortcut } from "./queue-shortcut";
...
export default async function QueuePage() {
  ...
  const entries = await loadQueue(session.user.id);
  const top = entries[0];
    ...
    <>
      ...
      <div className="flex-1 w-full max-w-[760px] mx-auto px-6 pt-10 pb-16">
        ...
        {top && (
          <QueueShortcut
            href={
              top.judgement.status === "awaiting_reply"
                ? `/judgements/${top.judgement.id}/thread`
                : `/reviews/${top.review.id}/judge?at=${top.judgement.ordinal}`
            }
          />
        )}

        <div className="border-t border-border">
          {entries.map(({ judgement, review, hasReply }, i) => {
            const isThread = judgement.status === "awaiting_reply";
            const tint = isThread ? REPLY_TINT : KIND_TINT[judgement.kind];
            const href = isThread
              ? `/judgements/${judgement.id}/thread`
              : `/reviews/${review.id}/judge?at=${judgement.ordinal}`;
